...
cp example.env .env
```
2.  **(Optional) Customize COnfigurations:**
...
docker compose up -d
```
4. **Verify Status:Ensure all containers are running and healthy:**
```bash
docker compose ps
```
5. **Configure Local Hosts:Map the GitLab hostname to your local machine** (replace \<gitlab-hostname\> with the value defined in your .env):
```bash
...
```bash
docker compose exec runner1 sh -c 'echo "network_mode = \"infrastructure_gitlab_net\"" >> /etc/gitlab-runner/config.toml'
```
